...
cardapio.metodos = {
    ...
    AdicionarAoCarrinho: (id) => {
        ...
        if (qntdAtual > 0) {
            ...
            if (item.length > 0) {
                ...
                let totalPrice = item[0].price; 
    
                // Caso já exista o item no carrinho, adicionar um novo índice
                if (existe) {
                    let novoId = id + '-' + (MEU_CARRINHO.length + 1); // Criar um novo ID único
                    let novoItem = {
                        ...item[0],
                        id: novoId,
                        qntd: qntdAtual,
                    ...
                    for (let i = 0; i < ITENS_ACOMP.length; i++) {
                        qntdAcomp = idAcompVariaveis[i].qntd;
    
                        if (qntdAcomp > 0) {
                            ITENS_ACOMP[i].qntd = qntdAcomp;
                            novoItem.acomp.push(ITENS_ACOMP[i]);

                            totalPrice += ITENS_ACOMP[i].price * qntdAcomp;
                        }
                    }

                    novoItem.totalPrice = totalPrice;
    
                    MEU_CARRINHO.push(novoItem);
                } else {
                    // Caso ainda não exista o item no carrinho, adiciona ele
                    item[0].qntd = qntdAtual;
                    item[0].comentario = $("#comentario-" + id).val();
                    ...
                    if (idAcompVariaveis.some(item => item.qntd > 0)) {
                        ...
                        for (let i = 0; i < ITENS_ACOMP.length; i++) {
                            ...
                            if (qntdAcomp > 0) {
                                ITENS_ACOMP[i].qntd = qntdAcomp;
                                item[0].acomp.push(ITENS_ACOMP[i]);
    ...
    editarItem: (id) => {
        ...
        var dataItens = {
            ...
            price: item[0].price,
            qntd: item[0].qntd,
            comentario: item[0].comentario
        };

        ITENS.push(dataItens);

        cardapio.metodos.carregarEditarlItem(ITENS[0], itensAcomp, item[0]);
    },
    ...
    //carrega a lista de descrição do item do carrinho
    carregarEditarlItem: (itens, itensAcomp, item) => {
        $("#descItensCarrinho").html('');
        $("#cardItensAcomp").html('');
        $("#containerFooterDesc").html('');
        $('#valor_total').val('');
    ...
    somarTotalItemDescEdit: (id) => {
        ...
        var qntdAtual = parseInt($("#qntd-carrinho-" + id).text());
        var valorAtualizado = (total * qntdAtual).toFixed(2).replace('.', ',');

        $("#totalDesc").text('R$ ' + valorAtualizado);
    },

    somarTotalAcompDescEdit: (id) => {
        let objIndex = ITENS_ACOMP.findIndex((obj => obj.id == id));

        var total = ITENS_ACOMP[objIndex].price;
        var qntdAtual = parseInt($("#qntd-acomp-" + id).text());
        var valorAtualizado = parseFloat((total * qntdAtual).toFixed(2).replace('.', ','));

        var totalAtual =  parseFloat($("#totalDesc").text().replace('R$ ', '').replace(',', '.'));
        var totalAtualizado = (totalAtual + valorAtualizado).toFixed(2).replace('.', ',');

        $("#totalDesc").text('R$ ' + totalAtualizado);
    },
    ...
    EditarAoCarrinho: (id) => {
        ...
        cardapio.metodos.atualizaBadgeTotal();
        cardapio.metodos.abrirCarrinho(true);
    },
    ...
    carregarCarrinho: () => {
        ...
        if (MEU_CARRINHO.length > 0) {
            ...
            $.each(MEU_CARRINHO, (i, e) => {
                ...
                $.each(e.acomp, (j, a) => {
                    acomp += `
                        <div class="col-3 col-qntd-acomp">
                            <span>${a.qntd}</span>
                        </div>
                        <div class="col-9 col-item-acomp">
                            <span>${a.name}</span>
                        </div>
                    `;
                });
                ...
                let temp = cardapio.templates.itemCarrinho.replace(/\${imagem}/g, e.img)
                    .replace(/\${nome}/g, e.name)
                    .replace(/\${preco}/g, e.totalPrice.toFixed(2).replace('.', ','))
                    .replace(/\${id}/g, e.id)
                    .replace(/\${qntd}/g, e.qntd)
                    .replace(/\${acomp}/g, acomp)
    ...
    carregarValores: () => {
        ...
        $.each(MEU_CARRINHO, (i, e) => {
            ...
            VALOR_CARRINHO += parseFloat(e.totalPrice * e.qntd);
...
cardapio.templates = {
    ...
    descItem: `
        ...
        <div class="col-12 col-lg-8 col-md-8 col-details-desc">
            ...
            <div class="container-comentario">
                <textarea id="comentario-\${id}" maxlength="140" placeholder="Ex: tirar a cebola, maionese à parte etc." oninput="cardapio.metodos.contarCaracteres('\${id}')"></textarea>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-12 col-lg-3 col-md-3">
                <div class="add-itens-acomp">
                    ...
                    <span class="add-numero-itens-desc add-numero-acomp" id="qntd-acomp-\${id}">\${qntd}</span>
    ...
    btnAdicionarItem: `
        <div class="add-itens-carrinho">
            ...
            <span class="add-numero-itens-carrinho" id="qntd-desc-\${id}">\${qntd}</span>
            <span class="btn-mais-carrinho" onclick="cardapio.metodos.aumentarQuantidadeDesc('\${id}')"><i class="fas fa-plus"></i></span>
        </div>

        <a id="addCart" onclick="cardapio.metodos.\${edit}AoCarrinho('\${id}')" class="btn btn-yellow btn-add-carrinho">
            <span class="spn-add">\${edit}</span> <span class="spn-total" id="totalDesc">R$ \${preco}</span>
        </a>
    `,
    ...
    itemCarrinho: `
        <div class="col-12 item-carrinho">
            <div class="container-item-carrinho">
                <div class="img-produto" onclick="cardapio.metodos.editarItem('\${id}')">
                    <div class="badge-edit-carrinho"><i class="fa-solid fa-pencil"></i></div>
                    <img src="\${imagem}" />
                </div>
